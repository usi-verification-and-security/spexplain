...
```
TIMEOUT=2m OPTIONS='--prefer-default-sample-neuron-activations=all --fix-default-sample-neuron-activations=all' ./scripts/run-experiments.sh explanations/obesity/short itp '^itp'
```
...
Additionally, it passes the arguments `--prefer-default-sample-neuron-activations=all --fix-default-sample-neuron-activations=all` to the underlying script `run1.sh` (i.e., consequently, to `spexplain`).
