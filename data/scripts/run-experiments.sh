...
function run1 {
    ...
    for rev in "${reverse_args[@]}"; do
        SRC_EXPERIMENT=$src_experiment "$DIRNAME/run.sh" "$OUTPUT_DIR" "$experiment_strategies" $experiment $rev $MAX_SAMPLES &
    done
    ...
    *)
        printf "%s failed!\nUsed command: %s\n" $experiment \
            "SRC_EXPERIMENT=$src_experiment \"$DIRNAME/run.sh\" \"$OUTPUT_DIR\" \"$experiment_strategies\" $experiment $rev $MAX_SAMPLES &" >&2
        return 1
        ;;
    esac
}
