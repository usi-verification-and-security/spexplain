...
// Parses a comma-separated list of numbers, e.g. "29,0,0,94,126,...". Used for the
// ONNX-only `--input-min`/`--input-max` options, since ONNX models carry no input domain
// information (unlike `.nnet` files, whose per-feature min/max are read from the file itself).
spexplain::Network2::Values parseFloatList(std::string_view sw) {
    spexplain::Network2::Values values;
    std::string const str{sw};
    std::istringstream iss{str};
    std::string token;
    while (std::getline(iss, token, ',')) {
        try {
            ...
            throw std::invalid_argument{"Expected a comma-separated list of numbers, got: "s + str};
        }
    }
    return values;
}
...
void printUsage(char * const argv[], std::ostream & os = std::cout) {
    ...
    os << "ACTIONS: [explain] explain-onnx dump-psi encode-onnx read-onnx\n";
    os << "ARGS:\n";
    os << "\t explain:\t <nn_model_fn> <dataset_fn> [<exp_strategies_spec>]\n";
    os << "\t explain-onnx:\t <onnx_model_fn> <dataset_fn> [<exp_strategies_spec>]\n";
    os << "\t dump-psi:\t <nn_model_fn>\n";
    os << "\t encode-onnx:\t <onnx_model_fn>\n";
    os << "\t read-onnx:\t <onnx_model_fn>\n";
    ...
    printUsageLongOptRow(os, "input-min", "<v1>,<v2>,...",
                         "ONNX only: per-feature input domain minimums (ONNX carries none; default: 0 for all)");
    printUsageLongOptRow(os, "input-max", "<v1>,<v2>,...",
                         "ONNX only: per-feature input domain maximums (ONNX carries none; default: 1 for all)");
    printUsageLongOptRow(os, "drop-sigmoid", "true|false",
                         "ONNX only: drop trailing sigmoid layer(s) so the output is the raw logit (default: true)");
    ...
    os << cmd << " data/models/heart_attack/heart_attack-50.onnx data/datasets/heart_attack/heart_attack_quick.csv\n";
    os << cmd << " explain data/models/toy.nnet data/datasets/toy.csv abductive -e data/explanations/toy.phi.txt\n";
    os << cmd << " explain-onnx data/models/heart_attack/heart_attack-50.onnx"
...
int mainExplainOnnx(int argc, char * argv[], int i, int nArgs);
...
std::optional<int> getOpts(int argc, char * argv[], spexplain::Framework::Config & config,
                           spexplain::Network2 & network2, std::string * explanationsFnPtr = nullptr) {
    ...
    constexpr int encodingReluLowerBoundsLongOpt = 15;
    constexpr int allowNeuronVarsInExplanationsLongOpt = 16;
    constexpr int inputMinLongOpt = 17;
    constexpr int inputMaxLongOpt = 18;
    constexpr int dropSigmoidLongOpt = 19;
    ...
    struct ::option longOptions[] = {
        ...
        {"filter-samples", required_argument, &selectedLongOpt, filterLongOpt},
        {"time-limit-per", required_argument, nullptr, 't'},
        {"input-min", required_argument, &selectedLongOpt, inputMinLongOpt},
        {"input-max", required_argument, &selectedLongOpt, inputMaxLongOpt},
        {"drop-sigmoid", required_argument, &selectedLongOpt, dropSigmoidLongOpt},
        {0, 0, 0, 0}};

    std::string optString = ":hV:E:e:s:vqRSIrn:i:t:";

    config.init2(network2);
            ...
            case 0: {
                ...
                switch (selectedLongOpt) {
                    ...
                    case inputPreferSampleNeuronActivationsLongOpt:
                        spexplain::parsePreferSampleNeuronActivations(config, optargStr);
                        break;
                    case inputMinLongOpt:
                        assert(hasArgument);
                        network2.setInputMinimums(parseFloatList(optargStr));
                        break;
                    case inputMaxLongOpt:
                        assert(hasArgument);
                        network2.setInputMaximums(parseFloatList(optargStr));
                        break;
                    case dropSigmoidLongOpt:
                        assert(hasArgument);
                        network2.setDropTrailingSigmoid(stringViewToBool(optargStr));
                        break;
                }
                break;
            }
...
int mainExplain(int argc, char * argv[], int i, int nArgs) {
    ...
    if (std::string_view const modelFn = argv[i + 1]; modelFn.ends_with(".onnx")) {
        return mainExplainOnnx(argc, argv, i, nArgs);
    }
...
int mainExplainOnnx(int argc, char * argv[], int i, int nArgs) {
    ...
    constexpr int minArgs = 2;
    if (nArgs < minArgs) {
        std::cerr << "Expected at least " << minArgs << " arguments for explain-onnx, got: " << nArgs << '\n';
        printUsage(argv, std::cerr);
        return 1;
    }

    std::string_view const onnxModelFn = argv[++i];
    auto networkPtr = spexplain::Network2::fromONNXFile(onnxModelFn);
    assert(networkPtr);
    ...
    auto dataset =
        spexplain::Network::Dataset{framework.getNetwork2().nInputs(), framework.getNetwork2().nClasses(), datasetFn};
    std::size_t const size = dataset.size();
...
int readEncodeONNX(int argc, char * argv[], int i, [[maybe_unused]] int nArgs) {

    assert(nArgs >= 1);

    std::string_view const nnModelFn = argv[++i];
    auto networkPtr = spexplain::Network2::fromONNXFile(nnModelFn);
    assert(networkPtr);
    ...
    spexplain::Framework framework{config};
    framework.setNetwork2(std::move(networkPtr));
    framework.setVerifier();
...
int readONNX(int argc, char * argv[], int i, [[maybe_unused]] int nArgs) {
    assert(nArgs >= 1);
    ...
    std::cout << "Network2 built with " << networkPtr->getLayers().size() << " layers.\n";
    return 0;
}
...
int main(int argc, char * argv[]) try {
    ...
    if (maybeAction == "explain-onnx") { return mainExplainOnnx(argc, argv, i, nArgs); }
    if (maybeAction == "dump-psi") { return mainDumpPsi(argc, argv, i, nArgs); }
    if (maybeAction == "read-onnx") { return readONNX(argc, argv, i, nArgs); }
    if (maybeAction == "encode-onnx") { return readEncodeONNX(argc, argv, i, nArgs); }
